...
export async function generateReleaseNotes(
  ...
  const markdown = releaseNotes
    ...
    : generateMarkdown(commits, fromCommit, to, last, options.group);
...
function generateMarkdown(
  ...
  // If groupTag is specified, combine all versions under that tag
  if (groupTag) {
    const allCommits: Commit[] = [];
    for (const version of sortedVersions) {
      const versionCommits = commitsByVersion.get(version) || [];
      allCommits.push(...versionCommits);
    }

    if (allCommits.length === 0) {
      markdown += `---\n\n`;
      markdown += `**Total:** 0 item(s)\n`;
      return markdown;
    }

    // Group all commits by section
    ...
    // Render single version header with groupTag
    markdown += `## ${groupTag}\n\n`;
